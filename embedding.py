...
def get_embeddings():
    ...
    compressor = models.resnet34(pretrained=None)
    compressor.fc = nn.Sequential(
        nn.Dropout(0.2),
        nn.Linear(compressor.fc.in_features, out_features=14),
        nn.Linear(14,7))
    compressor.load_state_dict(torch.load('Resnet.pt', map_location=torch.device('cpu')))
...
def similarity_checker(path,num_points):
    ...
    loaded_pca = joblib.load('pca_model_RS.pkl')
    ...
    embeddings = joblib.load('embeddings_RS.pkl')
